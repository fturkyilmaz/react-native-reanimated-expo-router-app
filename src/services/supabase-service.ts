...
interface SupabaseClientExtended extends SupabaseClient {
...
class SupabaseService {
  private client: SupabaseClientExtended | null = null;
  ...
  private initialize(): void {
    try {
      this.client = createClient() as SupabaseClientExtended | null;
   ...
   */
  private normalizeToIso(d: unknown): string | null {
  ...
  private buildMoviePayload(movie: Partial<Movie>) {
    ...
    const updatedAtIso = this.normalizeToIso(
      movie?.updated_at ?? movie?.updatedAt ?? null,
    );
  ...
  async addFavorite(
    ...
    if (!resolvedUserId || resolvedUserId === "local") {
      ...
      if (!authId) {
        console.error(
          "[SupabaseService] No authenticated user id available; cannot add favorite due to RLS.",
        );
        return false;
      }
      resolvedUserId = authId;
      console.log(
        "[SupabaseService] Resolved userId from auth:",
        resolvedUserId,
      );
    }
    ...
    try {
      ...
      const upsertError =
        (upsertResp as unknown as SupabaseResponse).error ?? null;
      const upsertData =
        (upsertResp as unknown as SupabaseResponse).data ?? null;
      ...
      const movieCheckError =
        (movieCheckResp as unknown as SupabaseResponse).error ?? null;
      const movieCheckData =
        (movieCheckResp as unknown as SupabaseResponse).data ?? null;
      ...
      if (
        !movieCheckData ||
        (Array.isArray(movieCheckData) && movieCheckData.length === 0)
      ) {
        console.warn(
          "[SupabaseService] Movie not found after upsert, aborting favorite insert",
          {
            movieId,
            movieCheckData,
          },
      ...
      // 3) Insert favorite
      console.log(
        "[SupabaseService] Inserting favorite for user:",
        resolvedUserId,
        "movie:",
        movieId,
      );
      ...
      const favError = (favInsert as unknown as SupabaseResponse).error ?? null;
      const favData = (favInsert as unknown as SupabaseResponse).data ?? null;

      console.log("[SupabaseService] Favorite insert result:", {
        favData,
        favError,
      });
      ...
      if (favError) {
        ...
        if (
          (favError.code === "23503" ||
            (favError.message &&
              favError.message.toLowerCase().includes("foreign key"))) &&
          !upsertError
        ) {
          console.log(
            "[SupabaseService] FK error detected. Retrying movie upsert and favorite insert once more...",
          );
          await this.client
            .from("movies")
            .upsert(moviePayload, { onConflict: "id" });
          const retryResp = await this.client
            .from("favorites")
            .insert({
              user_id: resolvedUserId,
              movie_id: movieId,
            ...
            .select();
          const retryError =
            (retryResp as unknown as SupabaseResponse).error ?? null;
          if (retryError) {
        ...
        if (
          favError.code === "42501" ||
          (favError.message &&
            favError.message.toLowerCase().includes("row-level security"))
        ) {
          console.error(
            "[SupabaseService] RLS violation. Ensure the favorites table has an insert policy like: with check (user_id = auth.uid()) and that the client is authenticated with the same user id.",
          );
        }
  ...
  async removeFavorite(userId: string, movieId: number): Promise<boolean> {
    ...
    try {
      ...
      if (!resolvedUserId) {
        console.error(
          "[SupabaseService] No authenticated user id available; cannot remove favorite due to RLS.",
        );
        return false;
      }
      ...
      const error = (resp as unknown as SupabaseResponse).error ?? null;
      const data = (resp as unknown as SupabaseResponse).data ?? null;
  ...
  async getFavorites(userId: string): Promise<unknown[]> {
    ...
    try {
      ...
      if (!resolvedUserId) {
        console.warn(
          "[SupabaseService] No authenticated user id available; returning empty favorites.",
        );
        return [];
      }
  ...
  async addToWatchlist(
    ...
    if (!resolvedUserId) {
      console.error(
        "[SupabaseService] No authenticated user id available; cannot add to watchlist due to RLS.",
      );
      return false;
    }
    ...
    try {
      ...
      const moviePayload = this.buildMoviePayload(movie);
      console.log(
        "[SupabaseService] moviePayload (watchlist upsert):",
        moviePayload,
      );
      ...
      if (upsertError) {
        console.error(
          "[SupabaseService] Error inserting movie to watchlist:",
          upsertError,
        );
      }
  ...
  async removeFromWatchlist(userId: string, movieId: number): Promise<boolean> {
    ...
    try {
      ...
      if (!resolvedUserId) {
        console.error(
          "[SupabaseService] No authenticated user id available; cannot remove from watchlist due to RLS.",
        );
        return false;
      }
      ...
      if (error) {
        console.error(
          "[SupabaseService] Error removing from watchlist:",
          error,
        );
        return false;
      }
  ...
  async getWatchlist(userId: string): Promise<unknown[]> {
    if (!this.isConfigured() || !this.client) {
      console.log("[SupabaseService] Not configured, returning empty");
      return [];
    }
    ...
    try {
      ...
      if (!resolvedUserId) {
        console.warn(
          "[SupabaseService] No authenticated user id available; returning empty watchlist.",
        );
        return [];
      }
  ...
  // Backup favorites to Supabase (bulk upsert)
  async backupFavorites(movies: Partial<Movie>[]): Promise<boolean> {
    console.log(
      "[SupabaseService] backupFavorites called with",
      movies.length,
      "movies",
    );

    if (!this.isConfigured() || !this.client) {
      console.log("[SupabaseService] Not configured, skipping backup");
      return false;
    }

    if (!movies || movies.length === 0) {
      console.log("[SupabaseService] No movies to backup");
      return true;
    }

    try {
      const resolvedUserId = await this.getAuthenticatedUserId();
      if (!resolvedUserId) {
        console.error(
          "[SupabaseService] No authenticated user id available; cannot backup favorites.",
        );
        return false;
      }

      // Build payloads for all movies
      const payloads = movies.map((movie) => ({
        user_id: resolvedUserId,
        movie_id: movie.id,
        title: movie.title ?? null,
        overview: movie.overview ?? null,
        poster_path: movie.poster_path ?? null,
        backdrop_path: movie.backdrop_path ?? null,
        release_date: this.normalizeToIso(movie.release_date),
        vote_average:
          typeof movie.vote_average === "number" ? movie.vote_average : null,
        genre_ids: Array.isArray(movie.genre_ids) ? movie.genre_ids : [],
      }));

      // Bulk upsert favorites
      const resp = await this.client
        .from("favorites")
        .upsert(payloads, { onConflict: ["user_id", "movie_id"] })
        .select();

      const error = (resp as unknown as SupabaseResponse).error ?? null;
      if (error) {
        console.error("[SupabaseService] Error backing up favorites:", error);
        return false;
      }

      console.log("[SupabaseService] Favorites backed up successfully!");
      return true;
    } catch (e) {
      console.error("[SupabaseService] Exception backing up favorites:", e);
      return false;
    }
  }

  // Backup watchlist to Supabase (bulk upsert)
  async backupWatchlist(movies: Partial<Movie>[]): Promise<boolean> {
    console.log(
      "[SupabaseService] backupWatchlist called with",
      movies.length,
      "movies",
    );
    ...
    try {
      ...
      if (!resolvedUserId) {
        console.error(
          "[SupabaseService] No authenticated user id available; cannot backup watchlist.",
        );
        return false;
      }
      ...
      const error = (resp as unknown as SupabaseResponse).error ?? null;
      if (error) {
        console.error("[SupabaseService] Error backing up watchlist:", error);
        return false;
      }

      console.log("[SupabaseService] Watchlist backed up successfully!");
      return true;
    } catch (e) {
      console.error("[SupabaseService] Exception backing up watchlist:", e);
      return false;
    }
  }

  // Restore favorites from Supabase
  async restoreFavorites(): Promise<unknown[]> {
    console.log("[SupabaseService] restoreFavorites called");
    ...
    try {
      ...
      if (error) {
        console.error("[SupabaseService] Error restoring favorites:", error);
        return [];
      }

      console.log(
        "[SupabaseService] Favorites restored successfully:",
        Array.isArray(data) ? data.length : 0,
        "items",
      );
      return (Array.isArray(data) ? data : []) || [];
    } catch (e) {
      console.error("[SupabaseService] Exception restoring favorites:", e);
      return [];
    }
  }

  // Restore watchlist from Supabase
  async restoreWatchlist(): Promise<unknown[]> {
    console.log("[SupabaseService] restoreWatchlist called");
    ...
    try {
      ...
      if (error) {
        console.error("[SupabaseService] Error restoring watchlist:", error);
        return [];
      }

      console.log(
        "[SupabaseService] Watchlist restored successfully:",
        Array.isArray(data) ? data.length : 0,
        "items",
      );
      return (Array.isArray(data) ? data : []) || [];
    } catch (e) {
      console.error("[SupabaseService] Exception restoring watchlist:", e);
      return [];
    }
  }
}
