...
class SupabaseAuthService {
  ...
  async signUp(
    ...
    try {
      ...
      if (error) {
        return { user: null, error: error.message as string };
      }
    ...
    try {
      ...
      if (data.user) {
        const user: User = {
          ...
          avatar: (data.user.user_metadata as { avatar_url?: string })
            ?.avatar_url,
          token: data.session?.access_token || "",
        };
    ...
    try {
      ...
      if (data.session?.user) {
        ...
        return {
          ...
          avatar: (user.user_metadata as { avatar_url?: string })?.avatar_url,
          token: data.session.access_token,
        };
      }
    } catch (error) {
