...
export const syncManager = {
  ...
  initialize: async (): Promise<void> => {
    ...
    try {
      ...
      logger.sync.info("Initialization complete", {
        isOnline: isConnected,
        pendingCount,
      });
    } catch (error) {
      logger.sync.error("Initialization failed", {
    ...
    try {
      ...
      for (const item of pendingItems) {
        ...
        try {
          ...
          synced++;
          logger.sync.info("Synced item", {
            id: item.id,
            type: item.type,
            operation: item.operation,
          });
        } catch (error) {
          logger.sync.error("Failed to sync item", {
            id: item.id,
            error: error instanceof Error ? error.message : error,
          });
      ...
      logger.sync.info("Sync complete", {
        synced,
        failed,
        pending: pendingCount,
      });
    } catch (error) {
      logger.sync.error("Sync error", {
        error: error instanceof Error ? error.message : error,
      });
    ...
    try {
      ...
      for (const item of unsynced) {
        try {
          ...
          logger.sync.error("Failed to sync favorite", {
      ...
      logger.sync.error("syncFavorites error", {
        error: error instanceof Error ? error.message : error,
      });
      return 0;
    }
    ...
    try {
      ...
      for (const item of unsynced) {
        try {
          ...
          logger.sync.error("Failed to sync watchlist", {
            movieId: item.movie_id,
            error: error instanceof Error ? error.message : error,
          });
        }
      ...
      logger.sync.error("syncWatchlist error", {
        error: error instanceof Error ? error.message : error,
      });
      return 0;
    }
  ...
  cleanupOrphanMovies: async (): Promise<number> => {
    try {
      ...
      logger.sync.error("cleanupOrphanMovies error", {
        error: error instanceof Error ? error.message : error,
      });
      return 0;
    }
