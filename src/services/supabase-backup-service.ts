    ...
    try {
      ...
      for (const movie of favorites) {
        try {
          ...
          console.warn(
            "[BackupService] Backup favorite failed:",
            movie.id,
            err,
          );
        }
      ...
      for (const movie of watchlist) {
        try {
          ...
          console.warn(
            "[BackupService] Backup watchlist failed:",
            movie.id,
            err,
          );
        }
  ...
  restore: async (): Promise<RestoreResult> => {
    ...
    try {
      ...
      for (const movie of favoritesData) {
        try {
          // Convert Supabase movie data to Movie type with proper defaults
          const rawMovie = movie as {
            movie_id?: number;
            id?: number;
            title?: string | null;
          ...
          const movieId = rawMovie.movie_id ?? rawMovie.id;
          if (typeof movieId !== "number") continue;
          ...
          const movieData: Movie = {
            ...
            vote_average: rawMovie.vote_average ?? 0,
            genre_ids: Array.isArray(rawMovie.genre_ids)
              ? rawMovie.genre_ids
              : typeof rawMovie.genre_ids === "string"
                ? JSON.parse(rawMovie.genre_ids)
                : [],
          };

          await FavoritesService.add(movieData, true);
          favoritesRestored++;
        } catch (err) {
          console.warn(
            "[BackupService] Restore favorite failed:",
            (movie as { movie_id?: number; id?: number }).movie_id ??
      ...
      for (const movie of watchlistData) {
        try {
          ...
          await WatchlistService.add(movieData, true);
          watchlistRestored++;
        } catch (err) {
          console.warn(
            "[BackupService] Restore watchlist failed:",
            (movie as { movie_id?: number; id?: number }).movie_id ??
              (movie as { id?: number }).id,
            err,
          );
